...
const getSlices = (data, ellipsized, scalar) => {
  const total = Object.keys(data).reduce((accum, key) => accum + data[key]['percent'], 0);
  ...
  Object.keys(data).forEach(key => {
    ...
    const percent = (data[key]['percent'] / total) * scalar;
    ...
    slices.push({
      key,
      labelTop: data[key]['name'],
      labelBottom: `${Math.round(percent * 10000) / 100}% ( ${data[key]['price']} )`,
      outerPath: [
        `M ${startX} ${startY}`,
...
class PieChartSVG extends PureComponent {
  ...
  render() {
    const { className, onToggle, onDeselect, svgRef } = this.props;
      ...
      <div className={className}>
        ...
        >
            ...
            {slices.map(({ key, labelTop, labelBottom, legend: [x, y], leaderLine, textAnchor }) => (
                <g key={key} className="chq-charts--noselect">
                    <line {...leaderLine} stroke="#666666" strokeWidth={0.01} />
                    <text x={x} y={y} textAnchor={textAnchor} fontSize={0.12}>
        ...
        {/* <ul className="noBulletsList">{
            Object.keys(data).map( (key, index) =>
                <li key={key}><span style={{backgroundColor: colors[index]}}> </span> {data[key]['name']}</li>
            )
        }</ul> */}
